...
test('map labels prioritize destinations and exclude overlaps and clipped names',()=>{
 ready();assert.equal(typeof UI.placeMapLabels,'function');
 const entries=[{text:'District',x:100,y:70,width:90,height:12,priority:1},{text:'Destination',x:100,y:70,width:100,height:12,priority:3},{text:'Far coast',x:200,y:110,width:70,height:12,priority:1},{text:'Clipped',x:5,y:10,width:70,height:12,priority:4}];
 const chosen=UI.placeMapLabels(entries,300,200);assert.deepEqual(chosen.map(p=>p.text),['Destination','Far coast']);assert.equal(entries[0].text,'District');
});
